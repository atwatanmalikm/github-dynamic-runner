...
mkdir -p ~/\${LABELID}
tar -xf actions-runner-linux-x64-2.293.0.tar.gz -C ~/\${LABELID}
cd ~/\${LABELID}
...
do	
	sleep 90
	BR=\$(hostname | cut -d- -f3)
        ID=\$(hostname | cut -d- -f4)
...
scp -o "StrictHostKeyChecking=no" label label_id worker.sh worker-exec.sh worker-rm-exec.sh gha worker-monitor.service  worker-monitor.sh gha@$(cat ip):~/
